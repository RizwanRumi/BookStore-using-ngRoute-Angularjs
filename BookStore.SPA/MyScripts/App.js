...
var MyApp = angular.module("MyApp", ['ngRoute', 'ui.bootstrap', 'AuthorService', 'BookService', 'FileService']);
...
.directive('authorMenu', function () {
    return {
        templateUrl: 'Views/templates/author-menu.html',
        restrict: 'E'
    }
});
...
MyApp.controller("BookController", ['$scope', '$window', '$http', '$routeParams', 'BookApi', 'FileApi', function ($scope, $window, $http, $routeParams, BookApi, FileApi) {
    ...
    $scope.getAuthorId = $routeParams.authorId;
    //var getId = $scope.getAuthorId;

    $scope.Books = [];

    $scope.Message = "";

    getBooks($scope.getAuthorId);

    function getBooks(getId) {
        
        $scope.TotalData = [];

        BookApi.getBooks(getId)
            .then(function (response) {
                $scope.TotalData = response.data;
    ...
    $scope.saveBook = function () {
            ...
            Boolean(price) == false && Boolean(pubhouse) == false && Boolean(fle) == false)
        {
            ...
            $scope.new.Book.AuthorsId = $scope.getAuthorId;
...
.directive('bookMenu', function () {
    return {
        templateUrl: 'Views/templates/book-menu.html',
        restrict: 'E'
    }
});;
